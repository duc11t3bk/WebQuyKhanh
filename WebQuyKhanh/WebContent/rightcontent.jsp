...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$("#btn-submit").on("click",function(){
			...
			var phoneNumber=$("#phoneNumberRS").val();
			...
			$.ajax({
				...
				success : function(response){
					...
					if("success"==result){
						$("#notify").html("Đăng ký thành công");
					}
					else{
						if(jsonObject.emailError!=null){
							$("#emailRS").css("border-color","red");
							$("#emailRSError").html(""+jsonObject.emailError);
						}
						if(jsonObject.phoneNumberError!=null){
							$("#phoneNumberRS").css("border-color","red");
							$("#phoneNumberRSError").html(""+jsonObject.phoneNumberError);
						}
					}
				},
		...
		$("#phoneNumberRS").focus(function(){
			resetStatus();
			$(this).css("border-color", "#66afe9");
			$("#phoneNumberRSError").css("visibility","visible");
			$("#phoneNumberRSError").css("opacity","1");
			setTimeout(function(){
				$("#phoneNumberRSError").css("opacity","0");
				$("#phoneNumberRSError").css("visibility","hidden");
			}, 1500);
		});
	});
</script>
...
<body>
	<div class="panel panel-primary mypanel">
		...
		<div class="panel-body form-register-study" style="text-align: center">
			<html:form action="/register-study">
				...
				<div class="row">
					<div class="col-lg-12 mytooltip">
						<html:text styleId="phoneNumberRS" property="phoneNumber" styleClass="form-control my-text-input" ></html:text>
						<span id="phoneNumberRSError" class="tooltiptext">Mời bạn nhập số điện thoại ! </span>
					</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<button id="btn-submit" type="button" class="my-btn">Đăng ký</button>
				</div>
			</html:form>
		</div>
	</div>
